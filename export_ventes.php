...
switch ($format) {
    case 'excel':
        ...
        foreach ($ventes as $vente) {
            ...
            if ($vente['type_vente'] === 'credit') {
                $total_verse += $vente['AccompteVerse'] ?? 0;
            } else {
        ...
        echo '<div class="summary-label">Total Versé/Acomptes</div>';
        echo '</div>';
        echo '<div class="summary-item">';
        ...
        echo '<th>💵 Montant Versé/Acompte</th>';
    ...
    case 'txt':
        ...
        $id = 1;
        foreach ($ventes as $vente) {
            $type_indicator = ($vente['type_vente'] === 'credit') ? ' (CRÉDIT)' : '';
            $montant_verse = ($vente['type_vente'] === 'credit') ? ($vente['AccompteVerse'] ?? 0) : $vente['MontantVerse'];
            $monnaie = ($vente['type_vente'] === 'credit') ? 0 : $vente['Monnaie'];
            
            $data .= str_pad($id++, 4) .
                     str_pad(substr($vente['NomPrenomClient'], 0, 24), 25) .
                     str_pad($vente['NumeroVente'] . $type_indicator, 20) .
                     str_pad(number_format($vente['MontantTotal'], 0, ',', ' ') . ' F', 15) .
                     str_pad(number_format($vente['MontantRemise'], 0, ',', ' ') . ' F', 12) .
                     str_pad(number_format($montant_verse, 0, ',', ' ') . ' F', 12) .
                     str_pad(number_format($monnaie, 0, ',', ' ') . ' F', 12) .
